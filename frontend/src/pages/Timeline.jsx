...
const Timeline = () => {
  ...
  const eraOptions = [
    ["all", t("timeline.era.all")],
    ["ancient", t("timeline.era.ancient")],
    ["medieval", t("timeline.era.medieval")],
    ["early-modern", t("timeline.era.earlyModern")],
    ["modern", t("timeline.era.modern")],
    ["contemporary", t("timeline.era.contemporary")],
  ];
    ...
    <div className="pt-32 pb-12 max-w-[1600px] mx-auto px-6 md:px-10" data-testid="timeline-page">
      ...
      <section className="mt-10 grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-gold/20 bg-gold/[0.05] p-5">
          <p className="overline text-gold">{t("timeline.corpus.label")}</p>
          <p className="mt-2 font-serif text-3xl text-bone">{figures.length}</p>
          <p className="mt-1 text-xs text-bone/50">{t("timeline.corpus.copy")}</p>
        </div>
        <div className="rounded-xl border border-bone/10 bg-bone/[0.025] p-5">
          <p className="overline">{t("timeline.span.label")}</p>
          <p className="mt-2 font-serif text-xl text-bone">{fmtYear(minY, t)} → {fmtYear(maxY, t)}</p>
          <p className="mt-1 text-xs text-bone/50">{t("timeline.span.copy")}</p>
        </div>
        <Link to="/atlas" className="rounded-xl border border-bone/10 bg-bone/[0.025] p-5 transition hover:border-gold/40">
          <p className="overline">{t("timeline.space.label")}</p>
          <p className="mt-2 font-serif text-xl text-bone">{t("timeline.space.title")}</p>
          <p className="mt-1 text-xs text-bone/50">{t("timeline.space.copy")}</p>
        </Link>
      </section>
      ...
      <div className="flex flex-wrap gap-3 mt-6" data-testid="timeline-legend">
        {LANES.map((cat) => (
          ...
          >
            ...
            {cat === "events" ? t("timeline.events") : t(`figures.${cat}`)}
          </button>
        ))}
      </div>
      ...
      <p className="text-bone/40 text-xs mt-4">{t("timeline.hoverHint")}</p>
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <Link to="/civilizations" className="rounded-xl border border-bone/10 p-5 hover:border-gold/40 transition">
          <p className="overline">{t("timeline.link.civilizations.label")}</p><p className="mt-2 font-serif text-lg text-bone">{t("timeline.link.civilizations.title")}</p>
        </Link>
        <Link to="/people" className="rounded-xl border border-bone/10 p-5 hover:border-gold/40 transition">
          <p className="overline">{t("timeline.link.peoples.label")}</p><p className="mt-2 font-serif text-lg text-bone">{t("timeline.link.peoples.title")}</p>
        </Link>
        <Link to="/diaspora" className="rounded-xl border border-bone/10 p-5 hover:border-gold/40 transition">
          <p className="overline">{t("timeline.link.diaspora.label")}</p><p className="mt-2 font-serif text-lg text-bone">{t("timeline.link.diaspora.title")}</p>
        </Link>
      </div>
    </div>
  );
