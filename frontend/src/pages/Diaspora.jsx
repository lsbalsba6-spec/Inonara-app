...
const TranslatedText = ({ value, className }) => {
  ...
  const display = translated || displayValue(value, lang);
...
const translatedRegion = (t, region, lang) => {
  if (!region) return "";
  if (typeof region === "string") {
    const translated = t(`region.${region}`);
    return translated === `region.${region}` ? region : translated;
  }
  return displayValue(region, lang);
};
...
export const DiasporaList = () => {
  ...
  const regions = useMemo(() => {
    ...
    items.map((d) => d.region).filter(Boolean).forEach((value) => {
      const key = searchableText(value);
      if (key && !unique.has(key)) unique.set(key, value);
    });
    return [...unique.values()].sort((a, b) =>
      translatedRegion(t, a, lang).localeCompare(translatedRegion(t, b, lang), lang === "en" ? "en" : "fr", { sensitivity: "base" }),
    );
  }, [items, lang, t]);
  ...
  const visible = useMemo(() => {
    ...
    const filtered = items.filter((d) => {
      const matchesRegion = region === "all" || searchableText(d.region) === searchableText(region);
  ...
  return (
    <div className="pt-32 pb-24 max-w-[1600px] mx-auto px-6 md:px-10" data-testid="diaspora-page">
      ...
      <p className="text-bone/70 max-w-2xl mt-6 font-light leading-relaxed">{t("page.diaspora.lead")}</p>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
          ...
          <div className="flex gap-2 overflow-x-auto">
            ...
            {regions.map((item, index) => {
              const key = stableKey(item, lang, `region-${index}`);
              const active = region !== "all" && searchableText(region) === searchableText(item);
              return <button key={key} type="button" onClick={() => setRegion(item)} aria-pressed={active} className={`whitespace-nowrap rounded-full border px-3 py-2 text-xs ${active ? "border-gold bg-gold/10 text-gold" : "border-bone/15 text-bone/60"}`}>{translatedRegion(t, item, lang)}</button>;
            })}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {visible.map((d, index) => {
          const name = displayValue(d.name, lang);
          const cardKey = d.id || stableKey(d.name, lang, `diaspora-${index}`);
          return (
            <Link key={cardKey} to={`/diaspora/${d.id}`} className="museum-card relative group overflow-hidden aspect-[4/5]" data-testid={`diaspora-card-${d.id}`}>
              <SmartImage src={d.image_url} wikipediaTitle={displayValue(d.wikipedia_title, lang)} alt={name} wrapperClassName="absolute inset-0" className="h-full w-full object-cover opacity-55 transition-all duration-1000 group-hover:scale-105 group-hover:opacity-75" credit={displayValue(d.image_credit, lang)} sourceUrl={d.image_source_url} />
              ...
              <div className="relative h-full p-7 flex flex-col justify-end">
                <p className="overline text-[0.65rem]">{translatedRegion(t, d.region, lang)}</p>
...
export const DiasporaDetail = () => {
  ...
  const name = displayValue(d.name, lang);
  const summary = tSummary || displayValue(d.summary, lang);
  const culture = tCulture || displayValue(d.culture, lang);
  const story = tStory || displayValue(d.story, lang);
  const modern = tModern || displayValue(d.modern, lang);
  const coords = Array.isArray(d.coords) && d.coords.length >= 2 && d.coords.every((value) => Number.isFinite(Number(value)))
    ? d.coords.map(Number)
    : null;

  const renderList = (values = [], prefix) => (
    <ul className="list-disc pl-5 space-y-1">
      {(Array.isArray(values) ? values : []).map((value, index) => (
        <li key={stableKey(value, lang, `${prefix}-${index}`)}><TranslatedText value={value} /></li>
      ))}
    </ul>
  );
  ...
  return (
    <div data-testid="diaspora-detail">
      <div className="relative h-[65vh] min-h-[440px] overflow-hidden">
        <SmartImage src={d.image_url} wikipediaTitle={displayValue(d.wikipedia_title, lang)} alt={name} wrapperClassName="absolute inset-0" className="h-full w-full object-cover animate-slow-zoom" credit={displayValue(d.image_credit, lang)} sourceUrl={d.image_source_url} />
        ...
        <div className="relative max-w-[1600px] mx-auto px-6 md:px-10 h-full flex flex-col justify-end pb-16">
          ...
          <p className="overline">{translatedRegion(t, d.region, lang)}</p>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-16">
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-6">
            {(d.origin_routes?.length ?? 0) > 0 && <Block label={t("section.origin_routes")}>{renderList(d.origin_routes, "route")}</Block>}
            {(d.ethnicities?.length ?? 0) > 0 && <Block label={t("section.ethnic_heritage")}>{renderList(d.ethnicities, "ethnicity")}</Block>}
          </div>
        </div>

        {((d.languages?.length ?? 0) > 0 || (d.religions?.length ?? 0) > 0) && (
          <div className="grid md:grid-cols-2 gap-10 mt-16">
            {(d.languages?.length ?? 0) > 0 && <Block label={t("section.languages")}>{renderList(d.languages, "language")}</Block>}
            {(d.religions?.length ?? 0) > 0 && <Block label={t("section.religions")}>{renderList(d.religions, "religion")}</Block>}
          </div>
        )}
        ...
        {culture && (
          <section className="mt-16 border-t border-[#2A2421] pt-10">
            ...
            <p className="text-bone/85 mt-5 text-lg font-light leading-relaxed max-w-3xl">{culture}</p>
          </section>
        )}
        ...
        {story && (
          <section className="mt-16 border-t border-[#2A2421] pt-10">
            ...
            <p className="text-bone/85 mt-5 text-lg font-light leading-relaxed max-w-3xl">{story}</p>
          </section>
        )}
        ...
        {modern && (
          <section className="mt-16 border-t border-[#2A2421] pt-10">
            ...
            <p className="text-bone/85 mt-5 text-lg font-light leading-relaxed max-w-3xl">{modern}</p>
          </section>
        )}
        ...
        {(d.image_credit || d.image_source_url) && (
          <section className="mt-10 rounded-2xl border border-bone/10 bg-bone/[0.025] p-6">
            ...
            {d.image_credit && <p className="mt-3 text-sm text-bone/65">{t("diaspora.credit")} : {displayValue(d.image_credit, lang)}</p>}
        ...
        {d.sources?.length > 0 && (
          <section className="mt-16 border-t border-[#2A2421] pt-10">
            ...
            <ul className="list-disc pl-5 space-y-2 mt-4 text-bone/70">
              {d.sources.map((source, index) => <li key={stableKey(source, lang, `source-${index}`)}><TranslatedText value={source} /></li>)}
            </ul>
          </section>
        )}
      </div>
