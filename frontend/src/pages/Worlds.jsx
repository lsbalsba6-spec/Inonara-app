  ...
  const worlds = [
    {
      id: "afroatlas",
      to: "/afroatlas",
      name: { en: "Afro Atlas", fr: "Afro Atlas" },
    ...
    { id: "asia",     name: { en: "Asia", fr: "Asie" },           subtitleKey: "worlds.coming.asia",     regionKey: "worlds.region.asia",     active: false },
    { id: "europe",   name: { en: "Europe", fr: "Europe" },       subtitleKey: "worlds.coming.europe",   regionKey: "worlds.region.europe",   active: false },
    { id: "americas", name: { en: "Americas", fr: "Amériques" }, subtitleKey: "worlds.coming.americas", regionKey: "worlds.region.americas", active: false },
    { id: "oceania",  name: { en: "Oceania", fr: "Océanie" },    subtitleKey: "worlds.coming.oceania",  regionKey: "worlds.region.oceania",  active: false },
  ];
  ...
  return (
    <div className="min-h-screen inonara-bg" data-testid="inonara-worlds">
      ...
      <header className="relative z-10 max-w-[1500px] mx-auto px-5 sm:px-8 md:px-12 pt-7 pb-4 flex items-center justify-between gap-3">
        <Link to="/" className="flex items-center gap-3 group" data-testid="worlds-brand-link" aria-label={`INONARA — ${t("nav.home")}`}>
      ...
      <section className="relative z-10 max-w-[1500px] mx-auto px-5 sm:px-8 md:px-12 pb-24" data-testid="worlds-grid">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-7">
          ...
          {worlds.filter((w) => w.active).map((w, i) => {
            const displayName = w.name[lang] || w.name.en;
            return (
            <Link
              key={w.id}
              to={w.to}
            ...
            >
              <img src={w.image} alt={displayName} className="absolute inset-0 w-full h-full object-cover opacity-65 group-hover:opacity-80 group-hover:scale-[1.04] transition-all duration-[1400ms]" />
              ...
              <div className="relative h-full p-6 sm:p-8 md:p-10 flex flex-col justify-end">
                ...
                <div className="flex items-center gap-4 mt-3">
                  ...
                  <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl text-[#f1e3c4] leading-[0.98] tracking-tight">{displayName}</h2>
                </div>
          ...
          {worlds.filter((w) => !w.active).map((w, i) => {
            ...
            >
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                ...
                <h3 className="font-serif text-2xl sm:text-3xl text-[#f1e3c4]/85 mt-3 tracking-tight">{displayName}</h3>
