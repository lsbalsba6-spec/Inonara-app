  ...
  return (
    <div className="space-y-8">
      <header className="rounded-xl border border-gold/20 bg-gold/[0.04] p-5">
        ...
        <h2 className="mt-2 font-serif text-3xl text-bone"><TranslatedInline value={data.title} /></h2>
        <p className="mt-2 max-w-3xl leading-relaxed text-bone/70"><TranslatedInline value={data.subtitle} /></p>
        <p className="mt-3 text-sm leading-relaxed text-bone/55"><TranslatedInline value={data.public_note} /></p>
      </header>
        ...
        {data.chapters?.map((chapter) => (
          <article key={chapter.id} className="rounded-xl border border-bone/10 bg-bone/[0.025] p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs uppercase tracking-widest text-gold"><TranslatedInline value={chapter.period} /></p>
              <StatusPill status={chapter.status} copy={copy} />
            </div>
            <h3 className="mt-2 font-serif text-2xl text-bone"><TranslatedInline value={chapter.title} /></h3>
            <p className="mt-3 leading-relaxed text-bone/75"><TranslatedInline value={chapter.text} /></p>
            <p className="mt-3 rounded-lg border border-bone/10 bg-black/10 p-3 text-xs leading-relaxed text-bone/55">
              <strong className="text-bone/70">{copy.mapRule}</strong>{" "}<TranslatedInline value={chapter.map_policy} />
            </p>
          ...
          {data.migration_processes?.map((process) => (
            <article key={process.id} className="rounded-xl border border-bone/10 p-4">
              <div className="flex items-start justify-between gap-3">
                <h3 className="font-serif text-lg text-bone"><TranslatedInline value={process.label} /></h3>
                <StatusPill status={process.status} copy={copy} />
              </div>
              <p className="mt-1 text-xs uppercase tracking-wider text-gold/80"><TranslatedInline value={process.period?.display} /></p>
              <p className="mt-3 text-sm leading-relaxed text-bone/70"><TranslatedInline value={process.reason} /></p>
