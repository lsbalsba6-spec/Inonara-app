...
function SourceCard({ source }) {
  return (
    <article className="rounded-2xl border border-bone/10 bg-bone/[0.025] p-5">
      ...
      {source.note && <p className="mt-4 text-sm leading-6 text-bone/65">{source.note}</p>}
      {source.url ? (
        <a href={source.url} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-full border border-gold/30 px-3 py-1.5 text-xs text-gold hover:bg-gold/10">
          Consulter la source
        </a>
      ) : (
  ...
  return (
    <div className="space-y-7">
      <header className="rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/[0.08] to-transparent p-6">
        ...
        <h2 className="mt-2 font-serif text-3xl text-bone">Ce qui est établi, discuté ou encore incertain</h2>
        <p className="mt-3 max-w-3xl leading-7 text-bone/65">
          Cette section distingue les faits largement établis, les interprétations concurrentes et les questions encore ouvertes.
        </p>
      </header>
      ...
      {!notes.length && <div className="rounded-xl border border-bone/10 p-5 text-bone/60">Aucun débat historiographique n’est encore renseigné.</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-7">
      <header className="rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/[0.08] to-transparent p-6">
        ...
        <h2 className="mt-2 font-serif text-3xl text-bone">Thèmes à enrichir ou vérifier</h2>
        <p className="mt-3 max-w-3xl leading-7 text-bone/65">
          Les lacunes sont signalées explicitement pour éviter de présenter un contenu incomplet comme définitif.
        </p>
      </header>
        ...
        {gaps.map((gap, index) => (
          <article key={index} className="rounded-2xl border border-bone/10 bg-bone/[0.025] p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="shrink-0 rounded-full border border-bone/15 px-2.5 py-1 text-[10px] uppercase tracking-wider text-bone/50">À suivre</span>
            </div>
          </article>
        ))}
      </div>
      {!gaps.length && <div className="rounded-xl border border-bone/10 p-5 text-bone/60">Aucun thème en attente n’est actuellement renseigné.</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-8">
      <header className="rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/[0.08] to-transparent p-6">
        ...
        <h2 className="mt-2 font-serif text-3xl text-bone">Bibliographie et références du dossier</h2>
        <p className="mt-3 max-w-3xl leading-7 text-bone/65">
          Les références sont classées par niveau de source et doivent rester traçables jusqu’à l’assertion qu’elles soutiennent.
        </p>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto]">
        ...
        <div className="flex gap-2 overflow-x-auto">
          <button type="button" onClick={() => setCategory("all")} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs ${category === "all" ? "border-gold bg-gold/10 text-gold" : "border-bone/15 text-bone/60"}`}>
            Toutes
          </button>
          {categories.map((itemCategory) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs ${String(category) === String(itemCategory) ? "border-gold bg-gold/10 text-gold" : "border-bone/15 text-bone/60"}`}
            >
              {CATEGORY_LABELS[itemCategory] || `Cat. ${itemCategory}`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {visible.map((source) => <SourceCard key={source.id} source={source} />)}
      </div>
      {!visible.length && <div className="rounded-xl border border-bone/10 p-5 text-bone/60">Aucune source ne correspond à cette recherche.</div>}
    </div>
  );
}
