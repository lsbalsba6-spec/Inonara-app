...
export function CountryMediaGallery({ items = [] }) {
  const categories = useMemo(() => [...new Set(items.map(getCategory))], [items]);
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const haystack = `${getTitle(item)} ${getCaption(item)} ${item.author || ""} ${item.license || ""}`.toLowerCase();
  ...
  return (
    <div className="space-y-8">
      <header className="rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/[0.08] to-transparent p-6">
        ...
        <h2 className="mt-2 font-serif text-3xl text-bone">Images, cartes et objets</h2>
        <p className="mt-3 max-w-3xl leading-7 text-bone/65">
          Chaque média conserve sa légende, son auteur, sa licence et sa page source afin de rester vérifiable.
        </p>
      </header>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((item, index) => (
          <figure key={item.id || `${getTitle(item)}-${index}`} className="overflow-hidden rounded-2xl border border-bone/10 bg-bone/[0.025]">
            ...
            <figcaption className="p-5">
              <p className="text-[10px] uppercase tracking-[0.18em] text-gold">{getCategory(item)}</p>
              <h3 className="mt-2 font-serif text-xl text-bone">{getTitle(item)}</h3>
              {getCaption(item) && <p className="mt-2 text-sm leading-6 text-bone/60">{getCaption(item)}</p>}
      ...
      {!visible.length && <div className="rounded-xl border border-bone/10 p-5 text-bone/60">Aucun média ne correspond à cette recherche.</div>}

      {selected && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 p-4" role="dialog" aria-modal="true" aria-label={getTitle(selected)}>
          <div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-bone/15 bg-[#151210]">
            <div className="flex items-center justify-between border-b border-bone/10 p-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-gold">{getCategory(selected)}</p>
                <h3 className="mt-1 font-serif text-2xl text-bone">{getTitle(selected)}</h3>
              </div>
              <button type="button" onClick={() => setSelected(null)} className="rounded-full border border-bone/15 px-3 py-1 text-sm text-bone/70">Fermer</button>
            </div>
            <SmartImage
            ...
            />
            <div className="p-5">
              {getCaption(selected) && <p className="leading-7 text-bone/68">{getCaption(selected)}</p>}
